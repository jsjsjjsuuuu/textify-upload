...
import { AlertTriangle, Check, Shield, Key, RotateCw, Save, Trash } from 'lucide-react';
...
const GeminiApiKeyManager: React.FC = () => {
  ...
  const { setGeminiApiKeys, getApiKeysInfo, setApiKeyModel } = useGeminiProcessing();
                ...
                <tbody>
                  {apiKeysInfo.map((info) => (
                    <tr key={info.keyId} className="border-b border-muted">
                      ...
                      <td className="p-2 text-center">{info.callCount}</td>
