...
const ExtractedDataField = ({
  ...
  return (
    <div className={`relative ${className} border rounded-md p-2 bg-gray-50 dark:bg-gray-900/60 hover:bg-gray-100 dark:hover:bg-gray-900/80 transition-colors`}>
        ...
        {editMode ? (
          isTextArea ? (
            <Textarea
              ...
              className="w-full text-sm resize-none h-16 bg-white dark:bg-gray-800"
            ...
            <div className="relative">
              <Input
                ...
                value={value}
                onChange={(e) => onChange(field, e.target.value)}
                className={`w-full text-sm bg-white dark:bg-gray-800 ${isLoading ? 'pr-8' : ''}`}
                placeholder={`أدخل ${label.replace(':', '')}`}
                disabled={isLoading}
              />
              {isLoading && (
          ...
          isTextArea ? (
            <div className={`px-3 py-2 rounded-md border bg-white dark:bg-gray-800 text-sm min-h-[4rem] whitespace-pre-wrap ${isLoading ? 'animate-pulse' : ''}`}>
              {isLoading ? (
            ...
            <div className={`px-3 py-2 rounded-md border bg-white dark:bg-gray-800 text-sm ${isLoading ? 'animate-pulse' : ''}`}>
              {isLoading ? (
