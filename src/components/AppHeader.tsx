    ...
    <header className="border-b bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60 sticky top-0 z-50">
      <div className="container max-w-6xl mx-auto flex h-16 items-center justify-between">
        ...
        <div className="flex items-center gap-6">
          <Link to="/" className="text-2xl font-medium text-primary">اصيل</Link>
              ...
              <>
                <Link 
                  ...
                  className={`transition-colors hover:text-foreground/80 ${
                    pathname === "/records" ? "text-foreground font-semibold" : "text-foreground/60"
                  }`}
                >
                  السجلات
                </Link>
                ...
                {isAdmin && (
                  <Link 
                    to="/admin/approvals" 
                    className={`transition-colors hover:text-foreground/80 ${
                      pathname === "/admin/approvals" ? "text-foreground font-semibold" : "text-foreground/60"
                    }`}
                  >
                    إدارة المستخدمين
                  </Link>
              ...
              <DropdownMenuTrigger asChild>
                <Button variant="ghost" className="h-9 w-9 p-0 rounded-full">
                  <Avatar className="h-9 w-9">
                    <AvatarImage 
                      src={userProfile?.avatar_url || `https://avatar.iran.liara.run/public/${user?.email}`} 
