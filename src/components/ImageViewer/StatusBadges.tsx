...
export const StatusBadges: React.FC<StatusBadgesProps> = ({ 
  ...
  const badges = [
    ...
    },
    {
      ...
      icon: <CheckCircle className="h-4 w-4 ml-2" />,
    ...
    },
    {
      ...
      label: "أخطاء",
      count: counts.error,
      icon: <AlertCircle className="h-4 w-4 ml-2" />,
  ...
  return (
    <div className="flex flex-wrap gap-4 justify-end mb-6" dir="rtl"> {/* زيادة المسافة بين العناصر */}
      {badges.map(badge => (
        <motion.div
          key={badge.id}
        ...
        >
          <Badge 
            ...
            className={`cursor-pointer border-2 text-base px-5 py-2 ${badge.className} ${activeFilter === badge.id ? badge.activeClassName : ""}`} /* زيادة حجم البادج */
          ...
          >
            ...
            <span className={`inline-flex items-center justify-center bg-white rounded-full w-6 h-6 text-sm mr-2 ${
