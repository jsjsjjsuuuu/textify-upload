...
export async function autoFillWebsiteForm(websiteUrl: string, data: any): Promise<ApiResult> {
  ...
  if (!newWindow) {
    return {
      success: false,
      message: "تم حظر النوافذ المنبثقة. يرجى السماح بالنوافذ المنبثقة للموقع."
    };
  }
    ...
    setTimeout(() => {
      try {
        ...
        if (newWindow.closed) {
          resolve({
            success: false,
            message: "تم إغلاق النافذة قبل اكتمال العملية"
          });
          return;
        }
        
        // إضافة كود جافاسكريبت للصفحة المستهدفة
        ...
        try {
          ...
          const messageListener = (event: MessageEvent) => {
            if (event.data && (event.data.type === 'FORM_FILL_RESULT' || event.data.type === 'FORM_FILL_ERROR')) {
              window.removeEventListener('message', messageListener);
              
              if (event.data.type === 'FORM_FILL_RESULT' && event.data.success) {
                resolve({
                  success: true,
                  message: `تم ملء ${event.data.filledFields} من ${event.data.totalFields} حقل بنجاح`,
                  data: {
                    url: websiteUrl,
                ...
                resolve({
                  success: false,
                  message: event.data.type === 'FORM_FILL_ERROR' 
                    ? `حدث خطأ أثناء ملء النموذج: ${event.data.error}`
                    : 'لم يتم العثور على حقول مناسبة للملء'
                });
              }
            }
          };
          ...
          // في حالة عدم تلقي رسالة خلال 10 ثوانٍ، نعتبر العملية نجحت
          setTimeout(() => {
            window.removeEventListener('message', messageListener);
            resolve({
              success: true,
              message: "تم تنفيذ عملية الإدخال (لم يتم تلقي تأكيد)",
              data: {
                url: websiteUrl,
          ...
          console.error("خطأ أثناء حقن الكود:", error);
          resolve({
            success: false,
